...
class AI_Core(QObject):
    ...
    async def listen_audio(self):
        ...
        self.audio_stream = pya.open(format=FORMAT, channels=CHANNELS, rate=SEND_SAMPLE_RATE, input=True, input_device_index=mic_info["index"], frames_per_buffer=SAMPLES_PER_FRAME)

        while self.is_running:
            # If AI is speaking, drain mic frames but don't send
            if speaking.is_set():
                try:
                    await asyncio.to_thread(self.audio_stream.read, SAMPLES_PER_FRAME, exception_on_overflow=False)
                except:
                    pass
                continue

            data = await asyncio.to_thread(self.audio_stream.read, SAMPLES_PER_FRAME, exception_on_overflow=False)
            if not self.is_running: break

            # Only send audio if VAD detects voice and AI is not speaking
            if self.vad_enabled and is_voiced(data):
                await self.out_queue_gemini.put({"data": data, "mime_type": "audio/pcm"})
            elif not self.vad_enabled:
                await self.out_queue_gemini.put({"data": data, "mime_type": "audio/pcm"})
    ...
    async def tts(self):
        ...
        while self.is_running:
            ...
            try:
                ...
                # Add tail buffer before re-enabling mic to avoid late reflections
